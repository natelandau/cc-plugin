...
and `config-protection`. Drop a file named after the hook
...
and `protected_files` / `protected_pyproject_tables` lists for `config-protection`.
A `protect-secrets` rule must target a named `field` (or
