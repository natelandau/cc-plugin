...
"""SessionEnd hook: trigger the end-of-session memory sweep.

Gating (lock, threshold, transcript window) runs inline; the heavy `claude -p`
pass runs in a detached worker that outlives session teardown. No-ops when
running inside the headless sweep agent or when the sweep is disabled.
Fail-open: any error exits 0 rather than wedging session end.
"""
