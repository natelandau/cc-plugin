...
The branch-protection hook resolves a branch by running `git rev-parse` with
the env it is handed, so any git-prefixed env var present in that env can
retarget git at a different repository than the test's own cwd (GIT_DIR and
GIT_WORK_TREE most directly, but also GIT_NAMESPACE and the
GIT_CONFIG_PARAMETERS / GIT_CONFIG_COUNT / GIT_CONFIG_KEY_<n> /
GIT_CONFIG_VALUE_<n> family, through which a config override such as
core.worktree has the same effect). The indexed
...
goes, including ones that only affect behavior, because there is no way to
keep those and still catch the indexed family.
...
def clean_environ(*, also_drop: frozenset[str] = frozenset()) -> dict[str, str]:
    ...
    Use for any env handed to a hook subprocess so git resolution targets the
    test's tmp project rather than the checkout the suite happens to run from.
    """
